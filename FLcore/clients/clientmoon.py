                ...
                if ottt:
                    ...
                    for t in range(self.timesteps):
                        ...
                        if task_id == 0:
                            ...
                            rep_old, out_fr_old = self.old_local_model(data, task_id, projection=False, update_hlop=flag, init=init)
                            rep_global, out_fr_global = self.global_model(data, task_id, projection=False, update_hlop=flag, init=init)
                        else:
                            rep, out_fr = self.local_model(data, task_id, projection=self.args.use_hlop, proj_id_list=[0], update_hlop=flag, fix_subspace_id_list=[0], init=init)
                            rep_old, out_fr_old = self.old_local_model(data, task_id, projection=self.args.use_hlop, proj_id_list=[0], update_hlop=flag, fix_subspace_id_list=[0], init=init)
                            rep_global, out_fr_global = self.global_model(data, task_id, projection=self.args.use_hlop, proj_id_list=[0], update_hlop=flag, fix_subspace_id_list=[0], init=init)
                    ...
                    if task_id == 0:
                        ...
                        rep, out = self.local_model(data, task_id, projection=self.args.use_hlop, proj_id_list=[0], update_hlop=flag, fix_subspace_id_list=[0])
                        rep_old, out_fr_old = self.old_local_model(data, task_id, projection=self.args.use_hlop, proj_id_list=[0], update_hlop=flag, fix_subspace_id_list=[0])
                        rep_global, out_fr_global = self.global_model(data, task_id, projection=self.args.use_hlop, proj_id_list=[0], update_hlop=flag, fix_subspace_id_list=[0])
